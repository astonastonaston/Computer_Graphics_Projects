...
// init viewer with renderer and osd_text 
...
void Viewer::update() {
  ...
  // at every display period
  // run user renderer
  // to re-display given frame buffer
  // only when param changed will that be re-rasterized
  if (renderer) {
    renderer->render();
  }

  // draw info
  // (drawing txts: frame rate info and renderer info)
  if( showInfo ) {
    drawInfo();        
  } 
...
void Viewer::drawInfo() {
  ...
  // define a bool to indicate need_update maybe ???? 
  if (renderer) {
...
// callback: re-render (change window) -> re-draw (change frame buffer)
