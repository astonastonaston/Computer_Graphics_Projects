...
// assignment(init) after colon!
// svgs: vector<SVG*>, for SVG file contents
...
// press key(callback): Change ras param -> re-draw -> re-render
// draw = proj + ras
 ...
 */
// update frame buffer
// redraw: re-transform-and-rasterize and update frame buffer
// update: frame buffer -> screen
// redraw = re-projection + re-rasterize
void DrawRend::redraw() {
  ...
  // transformation and rasterization, before drawing ?????
  // draw -> update frame buffer
  // texture rasterization here
  svg.draw(software_rasterizer, ndc_to_screen * svg_to_ndc[current_svg]);

  // printf("svgdr dne!!!\n");

  // manually draw canvas outline (Draw canvas edges)
