...
// read a given SVG file in and return read ptr
    ...
    while (((ent = readdir (dir)) != NULL) && (n < 9)) {
      ...
      if (filesufx == "svg" ) {
        ...
        if (!svg) {
          ...
          // load success: SVG file read content appended
          cerr << "Succeeded" << endl;
          svgs.push_back(svg);
          n++;
        }
...
vector<SVG*> loadPath( const char* path ) {
  ...
  if( st.st_mode & S_IFDIR ) {
    // returning a vec of SVG*s, corresponding to a vec of ptrs pting to file contents
    return loadDirectory(path);
  } 

  // load file
  if( st.st_mode & S_IFREG ) {
    // returning SVG* pting to file contents
...
int main( int argc, char** argv ) {
  ...
  // load svg file cotent, returning vector<SVG*>
  ...
  // for more than 4 args
  if (argc > 4 && strcmp(argv[2],"nogl") == 0) {
    app.init();
    app.set_gl(false);
    app.resize(stoi(argv[3]), stoi(argv[4]));
    // app.keyboard_event('=', 1, '0');
    // app.keyboard_event('=', 1, '0');
    // printf("samp rat %d\n", (app.software_rasterizer)->get_sample_rate());
    app.write_framebuffer();
    return 0;
  }

  // create viewer
  // viewer <- osd_texter*2, renderer
  ...
  // start viewer
  // by default: callbacks GL-based changing renderer
  viewer.start();
