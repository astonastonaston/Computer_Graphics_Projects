...
namespace CGL {
  ...
  void Vertex::computeCentroid( void ) {
    HalfedgeIter he_it = this->halfedge();
    Vector3D cent(0,0,0);
    int deg=0;
    do
    {
      he_it=he_it->twin();
      cent = cent + he_it->vertex()->position;
      deg++;
      he_it=he_it->next();
    } while (he_it != this->halfedge());
    this->centroid = cent / deg;
  }
      ...
      p++, f++ )
      {
        ...
        for( Index i = 0; i < degree; i++ )
        {
          ...
          if( pairToHalfedge.find( ab ) != pairToHalfedge.end() )
          {
            ...
            cerr << "This means that either (i) more than two faces contain this edge (hence the surface is non-manifold), or" << endl;
